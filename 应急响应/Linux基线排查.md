...
使用ssh-keygen，生成公钥和私钥

```shell
sudo ssh-keygen
```

![](https://ChihungSan.github.io/picx-images-hosting/linux_jixian/image.6m4g3welwg.webp)
...
然后将生成的私钥复制到本机（或者复制其中内容）id_rsa文件，直接指定使用私钥进行连接ssh即可

![](https://ChihungSan.github.io/picx-images-hosting/linux_jixian/image.3uvdvuhc5c.webp)

然后关闭ssh密码登录，修改`PasswordAuthentication no`

```shell
vim /etc/ssh/sshd_config
```

`systemctl restart ssh`重启ssh即可
...
> [!IMPORTANT]
>
> umask值用于设置用户在创建文件时的默认权限，如果 umask 值设置为 022，则新创建的文件将被设置为权限 644
...
将`*soft core 0`和`* hard core 0`前面的注释去掉

![](https://ChihungSan.github.io/picx-images-hosting/linux_jixian/image.73uhskxzee.webp)
